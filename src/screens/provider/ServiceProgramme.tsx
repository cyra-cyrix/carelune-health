...
/**
 * The registration link for ONE package.
 *
 ...
 * and is enrolled into exactly it. The token is opaque — the browser never
 * holds a package id, and the server re-resolves what the token means at
 * registration time.
 */
export function InviteControl({ pkg, enabled }: { pkg: ServicePackageRow; enabled: boolean }) {
            ...
            <PackageCard
              ...
              pricing={
                <>
                  ...
                  <InviteControl
                    pkg={service.packages[i]}
                    enabled={service.status === "published" && service.packages[i].status === "active"}
                  />
                </>
              }
            />
