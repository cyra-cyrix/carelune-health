...
import {
  updateOrgBranding, getMyInstitutionPathways, updatePathwayConfig, type InstitutionPathway,
} from "../../lib/db";
import {
  Card, Field, inputCls, PrimaryButton, GhostButton, Stepper, Chip, ErrorNote, Skeleton, SectionHeader,
 ...
 * editable commercial package fields. Finishing marks setup complete.
 */
export default function OrgSetup() {
  ...
  useEffect(() => {
    void getMyInstitutionPathways().then(setPacks).catch(() => setPacks([]));
  }, []);
    ...
    <div className="min-h-screen bg-mist">
      ...
      <main className="mx-auto max-w-[820px] px-5 py-7">
        ...
        <div className="mt-5">
          ...
          {step === 1 && (
            <Card>
              <SectionHeader title="Your programmes" sub="Assigned by Carelune. Clinical content is governed and read-only." />
              <div className="mt-4 space-y-3">
                {packs === null ? (
                  <><Skeleton className="h-20" /><Skeleton className="h-20" /></>
                ) : packs.length === 0 ? (
                  <p className="text-[13px] text-sage-500">No programmes assigned yet. Ask Carelune to enable a Continuum Care pack.</p>
                ) : (
                  packs.map((p) => (
                    <div key={p.pack_id} className="rounded-2xl border border-line bg-white p-4">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <Chip tone="warn">Requires clinical approval</Chip>
                      </div>
          ...
          {step === 2 && (
            <Card>
              <SectionHeader title="Programme & package" sub="Set the price families see. Clinical content stays governed." />
              <div className="mt-4 space-y-4">
                {packs === null ? (
                  <Skeleton className="h-40" />
                ) : packs.length === 0 ? (
                  <p className="text-[13px] text-sage-500">No programmes to price yet.</p>
                ) : (
                  packs.map((p) => <PackageEditor key={p.pack_id} pack={p} />)
                )}
                {error && <ErrorNote>{error}</ErrorNote>}
                <div className="flex justify-between pt-1">
          ...
          {step === 3 && (
            <Card>
              ...
              <ul className="mt-4 space-y-2 text-[13.5px] text-sage-700">
                ...
                <li className="flex gap-2"><span className="text-sky-600">✓</span> {packs?.length ?? 0} programme{(packs?.length ?? 0) === 1 ? "" : "s"} reviewed and priced</li>
...
function PackageEditor({ pack }: { pack: InstitutionPathway }) {
  ...
  return (
    <div className="rounded-2xl border border-line bg-white p-4">
      ...
      <div className="mt-3 grid gap-3 sm:grid-cols-3">
        ...
        <Field label="Programme duration (days)">
